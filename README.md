...
Pocket Bricks is a Progressive Web App, so users can install it from the browser without Play Store approval.

### Android

1. Open the live demo URL in Chrome, Edge, Brave, or another modern Android browser.
2. Tap **INSTALL APP** inside the game if the install prompt appears.
3. If no prompt appears, open the browser menu and choose **Install app** or **Add to Home screen**.
4. Launch **Pocket Bricks** from the home screen.
...
4. Keep the name **Pocket Bricks** and tap **Add**.

Full installation notes are in [INSTALL.md](./INSTALL.md).
...
- Local memory saved in the browser: best score, last score, games played, best level, best lines, and sound preference.
- Optional square-wave beeps generated by Web Audio.
- Installable PWA with offline play.
- In-game install prompt for supported Android browsers.
...
├── src/
│   ├── app.js               # Browser, canvas, controls, local memory, sound, PWA integration
...
2. **App shell** — `src/app.js` handles canvas drawing, touch controls, keyboard controls, local memory, sound, service worker registration, and install prompts.
